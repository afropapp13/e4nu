...
void AbsXSec_OverlayECal_FineBinning_Pilar() {
...
//	NameOfPlots.push_back("h_Etot_subtruct_piplpimi_2p1pi_1p0pi_fracfeed"); 
	...
	for (int WhichxBCut = 0; WhichxBCut < NxBCuts; WhichxBCut ++) {
		...
		TFile* fXSecFile = TFile::Open("myXSec/FineBin_ECalReso_NormYield_"+xBCut[WhichxBCut]+".root","recreate");
		...
		for (int WhichPlot = 0; WhichPlot < NPlots; WhichPlot ++) {
			...
			for (int WhichEnergy = 0; WhichEnergy < NEnergies; WhichEnergy ++) {
				...
				for (int WhichNucleus = 0; WhichNucleus < NNuclei; WhichNucleus ++) {
					...
					if (nucleus[WhichNucleus] == "56Fe") { MaxHeight = 2.6; }
...
//						ReweightPlots(Plots[WhichFSIModel]);
						...
						if (DoubleE[WhichEnergy] == 4.461) { Plots[WhichFSIModel]->Scale(4.); }
						...
						if (
							...
							for (int j = 1; j < 5; j++) {
								...
								if (DoubleE[WhichEnergy] == 4.461) { BreakDownPlots[j-1]->Scale(4.); }
